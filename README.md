...
1. Завантаж готовий файл `com.inq.marks_<version>.wotmod`.
2. Скопіюй його до:

   ```text
   <World of Tanks>/mods/<версія_клієнта>/
   ```

3. Запусти або перезапусти гру.

Після першого запуску конфігурація створюється тут:

```text
<World of Tanks>/mods/configs/inq/marks/marks.json
```
