...
required_paths = [
    ...
    "build.py",
    "build.example.json",
    "python/gui/mods/mod_inq_marks.py",
    "as3/src_flash/InqMarksPanelHangar.as3proj",
    "as3/src_flash/InqMarksPanelBattle.as3proj",
    "resources/in/mods/inq.marks/en.json",
    "resources/in/mods/inq.marks/ru.json",
    "resources/in/mods/inq.marks/uk.json",
]
...
mod_python = ROOT / "python/gui/mods/mod_inq_marks.py"
...
for path in iter_files("*.as"):
    ...
    as3_references.update(
        re.findall(r"\b(?:new|extends|implements)\s+(InqMarks[A-Za-z0-9_]*|InqMark[A-Za-z0-9_]*)", clean)
    )
...
for path in list(iter_files("*.py")) + list(iter_files("*.as")) + list(iter_files("*.as3proj")):
    try:
        text = path.read_text(encoding="utf-8-sig", errors="ignore")
    except OSError:
        continue
    for token in forbidden_source_tokens:
        if token in text:
            add_error(path, "standalone Marks source contains a forbidden cross-module token: %s" % token)


# INQ branding and isolation checks.
legacy_brand_tokens = (
    "under" + "_pressure",
    "under" + "-pressure",
    "me." + "under" + "_pressure",
    "com." + "under" + "_pressure",
    "mod_" + "under" + "_pressure",
    "mods/" + "under" + "_pressure",
)
brand_text_suffixes = {".as", ".as3proj", ".json", ".md", ".py", ".ps1", ".yml", ".yaml"}
for path in ROOT.rglob("*"):
    if is_ignored(path) or not path.is_file() or path.suffix.lower() not in brand_text_suffixes:
        continue
    try:
        source = path.read_text(encoding="utf-8-sig", errors="ignore")
    except OSError:
        continue
    for token in legacy_brand_tokens:
        if token in source:
            add_error(path, f"legacy brand token remains: {token}")

expected_build = json.loads((ROOT / "build.example.json").read_text(encoding="utf-8-sig"))
if expected_build.get("info", {}).get("id") != "com.inq.marks":
    add_error("build.example.json", "mod id must be com.inq.marks")
if expected_build.get("info", {}).get("name") != "INQ Marks":
    add_error("build.example.json", "mod name must be INQ Marks")

required_inq_tokens = {
    ROOT / "python/gui/mods/mod_inq_marks.py": ("inq.marks", "InqMarksController", "InqMarksPanelInjectorView"),
    ROOT / "as3/src_flash/src/com/inq/marks/InqMarksPanelInjector.as": ("package com.inq.marks", "InqMarksPanelInjector"),
}
for path, tokens in required_inq_tokens.items():
    if not path.is_file():
        continue
    source = path.read_text(encoding="utf-8-sig", errors="ignore")
    for token in tokens:
        if token not in source:
            add_error(path, f"required INQ token is missing: {token}")
