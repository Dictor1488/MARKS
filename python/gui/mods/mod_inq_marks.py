...
logger = logging.getLogger('inq.marks')
logger.setLevel(logging.DEBUG if _DEBUG else logging.ERROR)

__version__ = '0.1.0'

_LINKAGE_HANGAR = 'InqMarksPanelHangar'
_LINKAGE_BATTLE = 'InqMarksPanelBattle'
_SWF_HANGAR = 'InqMarksPanelHangar.swf'
_SWF_BATTLE = 'InqMarksPanelBattle.swf'

_L10N_DIR = 'mods/inq.marks'
_L10N_FALLBACK = 'en'
_l10n = {}

_API_APP_ID = 'bce57ac20af6b67b08be09fd66847ed9'
_API_URL_TEMPLATE = (
    'https://api.worldoftanks.%s/wot/tanks/' + 'ma' + 'stery/'
...
_CACHE_DIR = os.path.normpath(os.path.join(os.path.dirname(_prefsFilePath), 'mods', 'inq', 'marks'))
_CACHE_FILE = os.path.join(_CACHE_DIR, 'cache.dat')
_CONFIG_DIR = os.path.normpath(os.path.join(os.getcwd(), 'mods', 'configs', 'inq', 'marks'))
_CONFIG_FILE = os.path.join(_CONFIG_DIR, 'marks.json')
_CACHE_VERSION = 11
_CACHE_TTL_SECONDS = 3 * 24 * 3600
_CACHE_SAVE_DEBOUNCE = 3.0
_VALID_BATTLE_STYLES = ('classic', 'compact', 'polaroid', 'minimal')
_GARAGE_STYLES = ('classic', 'compact', 'polaroid')
_DEFAULT_BATTLE_STYLE = 'classic'
_CONFIG_STYLE_HINT = 'classic | compact | polaroid | minimal'
...
def _replayValue(controller, name, default=None):
    try:
        value = getattr(controller, name, default)
        if callable(value):
            value = value()
        return value
    except Exception:
...
def _isReplayPlaying():
    try:
        ...
        if g_replayCtrl is None:
            return False
        for name in ('isPlaying', 'isReplayPlaying'):
            if bool(_replayValue(g_replayCtrl, name, False)):
                return True
        return False
    except Exception:
        return False
...
def _getReplayPlayerVehicleID():
    try:
        from BattleReplay import g_replayCtrl
        if g_replayCtrl is None:
            return 0
        for name in ('playerVehicleID', 'playerVehicleId',
                     'getPlayerVehicleID', 'getPlayerVehicleId'):
            value = _replayValue(g_replayCtrl, name, 0)
            try:
...
def _buildMoeApiUrl(tankID):
    # WG exposes MoE damage percentiles through its official distribution endpoint.
    return _API_URL_TEMPLATE % (
        _getApiDomain(), 'damage', _MOE_PERCENTILES_QUERY, tankID)
...
def _parseMoeApiResponse(payload, tankID):
    """Extract MoE damage percentile thresholds for one tank."""
    ...
    if not isinstance(record, dict):
        return None
    result = {}
    for percentile, key in _MOE_PERCENTILE_TO_KEY:
...
class InqMarksSessionHistory(object):
...
class InqMarksPanelInjectorView(View):
    _g_controller = None
    _viewKind = 'hangar'

    def _populate(self):
        super(InqMarksPanelInjectorView, self)._populate()
        if InqMarksPanelInjectorView._g_controller:
            InqMarksPanelInjectorView._g_controller._onInjectorReady(self)

    def _dispose(self):
        if InqMarksPanelInjectorView._g_controller:
            InqMarksPanelInjectorView._g_controller._onInjectorDisposed(self)
        super(InqMarksPanelInjectorView, self)._dispose()

    def py_onDragEnd(self, offset):
        if InqMarksPanelInjectorView._g_controller:
            InqMarksPanelInjectorView._g_controller._onDragEnd(offset)

    def py_onPanelReady(self):
        if InqMarksPanelInjectorView._g_controller:
            InqMarksPanelInjectorView._g_controller._onPanelReady(self)


    def py_onMarkBadgeToggle(self, value):
        if InqMarksPanelInjectorView._g_controller:
            InqMarksPanelInjectorView._g_controller._onMarkBadgeToggle(value)

    def py_onMarkBadgeOffsetChanged(self, offset):
        if InqMarksPanelInjectorView._g_controller:
            InqMarksPanelInjectorView._g_controller._onMarkBadgeOffsetChanged(offset)

    def py_onBattleBadgeOffsetChanged(self, offset):
        if InqMarksPanelInjectorView._g_controller:
            InqMarksPanelInjectorView._g_controller._onBattleBadgeOffsetChanged(offset)


class InqMarksPanelHangarView(InqMarksPanelInjectorView):
    _viewKind = 'hangar'


class InqMarksPanelBattleView(InqMarksPanelInjectorView):
    _viewKind = 'battle'


def _registerFlash():
    for linkage, viewCls, swf in ((_LINKAGE_HANGAR, InqMarksPanelHangarView, _SWF_HANGAR),(_LINKAGE_BATTLE, InqMarksPanelBattleView, _SWF_BATTLE)):
...
class InqMarksController(object):
    ...
    def _updateBattleVisibility(self):
        ...
        logger.debug('battle visibility: visible=%s replay=%s mode=%s tankID=%s hidden=%s',
                     visible, _isReplayPlaying(), self._battleMode, self._battleTankID,
    ...
    def _installInputHandlerHook(self):
        ...
        try:
            ...
            if orig is None:
                ...
                return
            if getattr(orig, '_marks_hooked', False):
                return
    ...
    _EMPTY_MOE = {
        'p20': 0, 'p40': 0, 'p55': 0, 'p60': 0, 'p65': 0,
        'p70': 0, 'p75': 0, 'p85': 0, 'p95': 0, 'p100': 0,
    }
    ...
    def _loadCache(self):
        ...
        try:
            ...
            # Older combined caches are accepted, but unrelated percentile
            # entries are deliberately ignored and never written back.
            ...
            logger.debug(
                'cache loaded v%s -> v%s: %d moe, pos=%s, history-accounts=%d',
                version, _CACHE_VERSION, len(self._moeCache), self._position,
    ...
    def _saveCache(self, rev=None):
        ...
        try:
            ...
            raw = zlib.compress(
                cPickle.dumps((payload, _CACHE_VERSION), cPickle.HIGHEST_PROTOCOL), 1)
            with open(_CACHE_FILE, 'wb') as fh:
                fh.write(raw)
            logger.debug('cache saved: %d moe', len(self._moeCache))
        except Exception:
            logger.exception('cache: failed to save')

    def _isFresh(self, tankID):
        moe = _dictGetTank(self._moeCache, tankID)
        if moe is None or not _hasFullMoeRequirements(moe):
            return False
        ts = _dictGetTank(self._moeCacheTs, tankID, 0)
        try:
    ...
    def _refresh(self):
        ...
        if tankLevel and tankLevel < _MIN_TANK_LEVEL:
            logger.debug(
                'refresh: hiding panel, tank level %d < %d',
                tankLevel, _MIN_TANK_LEVEL)
            self._visibleByData = False
            self._updateVisibility()
            try:
                self._injectorView.flashObject.as_clearData()
        ...
        self._updateVisibility()
        logger.debug(
            'refresh: tankID=%s visibleByData=True hangarVisible=%s',
        ...
        if not self._isFresh(tankID):
            self._requestMoe(tankID)
    ...
    def _requestMoe(self, tankID, attempt=1):
        """Request MoE damage percentile thresholds from the WG API."""
        if attempt == 1:
            if tankID in self._pendingMoe:
                return
            self._pendingMoe.add(tankID)
        url = _buildMoeApiUrl(tankID)
        logger.debug(
            'moe api request tankID=%s attempt=%d url=%s', tankID, attempt, url)
        try:
            BigWorld.fetchURL(
                url,
                lambda response, t=tankID, a=attempt: self._onMoeApiResponse(t, response, a),
                None, _API_TIMEOUT, 'GET', None,
            )
        except Exception:
            logger.exception(
                'moe fetchURL failed tankID=%s attempt=%d', tankID, attempt)
            self._handleMoeApiFailure(tankID, attempt)

    def _retryMoeRequest(self, tankID, attempt):
        if not self._enabled:
            self._pendingMoe.discard(tankID)
            return
        self._requestMoe(tankID, attempt)

    def _handleMoeApiFailure(self, tankID, attempt):
        if attempt < _API_MAX_ATTEMPTS:
            delay = _API_RETRY_BASE_DELAY * (2 ** (attempt - 1))
            nextAttempt = attempt + 1
            logger.debug(
                'moe api retry tankID=%s in %.1fs (next attempt=%d)',
                tankID, delay, nextAttempt)
            BigWorld.callback(
                delay, lambda: self._retryMoeRequest(tankID, nextAttempt))
            return

        self._pendingMoe.discard(tankID)
        logger.debug(
            'moe api: gave up tankID=%s after %d attempts', tankID, attempt)
        ...
        if isCurrent and _dictGetTank(self._moeCache, tankID) is None:
            self._pushMoe(self._EMPTY_MOE)

    def _onMoeApiResponse(self, tankID, response, attempt=1):
        parsed = None
        status = 0
        try:
            ...
            if body and status and status < 400:
                parsed = _parseMoeApiResponse(json.loads(body), tankID)
        except Exception:
            logger.exception(
                'moe api parse failed tankID=%s attempt=%d', tankID, attempt)
        ...
        if parsed is None:
            ...
            if isTransient and attempt < _API_MAX_ATTEMPTS:
                ...
                return
            self._pendingMoe.discard(tankID)
            logger.debug('moe api: no data tankID=%s status=%s', tankID, status)
            current = g_currentVehicle.item if g_currentVehicle.isPresent() else None
            isCurrent = current is not None and getattr(current, 'intCD', None) == tankID
            if isCurrent and _dictGetTank(self._moeCache, tankID) is None:
                self._pushMoe(self._EMPTY_MOE)
...
class _StrongholdBrowserWatcher(object):
    ...
    def _patchClass(self, cls):
        ...
        for methodName in dir(cls):
            ...
            try:
                ...
                continue
            if not callable(original) or getattr(original, '_marksStrongholdWrapped', False):
                continue
    ...
    def _makeWrapper(self, methodName, original):
        ...
        _wrapped._marksStrongholdWrapped = True
        return _wrapped
...
class _InqMarksMod(object):

    def __init__(self):
        self._session = InqMarksSessionHistory()
        self._ctrl = InqMarksController(session=self._session)
        ...
        InqMarksPanelInjectorView._g_controller = self._ctrl
    ...
    def fini(self):
        ...
        self._results.fini()
        self._ctrl.disable()
        InqMarksPanelInjectorView._g_controller = None
        _unregisterFlash()
    ...
    def _onLobbyAppInitialized(self, event):
        try:
            ...
            app.loadView(SFViewLoadParams(_LINKAGE_HANGAR))
            logger.debug('marks: injector reloaded on SF_LOBBY init')
        except Exception:
            self._ctrl._injectPending = False
            logger.exception('marks: failed to reload injector on SF_LOBBY init')
    ...
    def _tryEnterBattle(self, attempt):
        ...
        if tankID is None:
            if attempt < 30:
                ...
                logger.debug('battle enter: no tankID after %d attempts replay=%s',
                             attempt, _isReplayPlaying())
            return
        self._battleEnterCallbackId = None
        if self._ctrl._battleMode and self._ctrl._battleTankID == tankID:
            return
        isReplay = _isReplayPlaying()
        try:
            player = BigWorld.player()
            ...
            for attr in ('RANDOM', 'MAPBOX'):
                ...
                if val is not None:
                    allowed.append(val)
            if not isReplay and allowed and guiType not in allowed:
                logger.debug('battle enter: skipped guiType=%s (not RANDOM/MAPBOX)', guiType)
                return
        except Exception:
            pass
        logger.debug('battle enter: accepted tankID=%s replay=%s', tankID, isReplay)
    ...
    def _snapshotBaselineForBattle(self):
        ...
        if tankID is None:
            try:
                ...
                if not pvid and _isReplayPlaying():
                    pvid = _getReplayPlayerVehicleID()
...
def _installVehicleDamageHooks():
    ...
    Wraps the original method and forwards calls to _InqMarksMod
    ...
    _VEHICLE_HOOK_MOD_REF = weakref.ref(_g_inq_marks)
